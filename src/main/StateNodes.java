...
public class StateNodes {
	ArrayList<String> stateConnections = new ArrayList<String>();
	...
	public String find_Connection_DFA(String element) {
		...
		int i = 0;
		if (stateConnections.isEmpty()) {
			return "false";
		}
		while ((i < stateConnections.size()) && (!found)) {
			temp = stateConnections.get(i);
		...
		if (found) {
			element = stateConnections.get(i);
