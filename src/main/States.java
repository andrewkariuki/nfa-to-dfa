...
public class States {
	...
	public void print_States_And_Connections() {
		...
		for (int i = 0; i < states.size(); i++) {
			...
			for (int t = 0; t < states.get(i).stateConnections.size(); t++) {
				String temp = states.get(i).stateConnections.get(t);
	...
	public void print_SORTER() {
		...
		for (int i = 0; i < diagram_Sort.size(); i++) {
			...
			for (int t = 0; t < diagram_Sort.get(i).stateConnections.size(); t++) {
				String temp = diagram_Sort.get(i).stateConnections.get(t);
				System.out.print(temp.charAt(0) + " connected to: " + temp.substring(1) + "\n");
			}
		}
	}
	...
	public void print_NFA_Nodes() {
		...
		for (int i = 0; i < NFA_State_Nodes.size(); i++) {
			...
			for (int t = 0; t < NFA_State_Nodes.get(i).stateConnections.size(); t++) {
				String temp = NFA_State_Nodes.get(i).stateConnections.get(t);
				System.out.print(temp.charAt(0) + " connected to: " + temp.substring(1) + "\n");
			}
		}
	}
	...
	public void build_NFA_States() {
		...
		for (int i = 0; i < states.size(); i++) {
			for (int t = 0; t < states.get(i).NFA_States.size(); t++) {
				...
				for (int k = 0; k < states.get(location).stateConnections.size(); k++) {
					String element = states.get(location).stateConnections.get(k);
	...
	public void remove_Connection_From_One(int state, String element) {
		for (int k = 0; k < states.get(state).stateConnections.size(); k++) {
			String temp = states.get(state).stateConnections.get(k);
			temp = temp.substring(0, 1);
			if (temp.equals(element)) {
				states.get(state).stateConnections.remove(k);
	...
	public void sort_States(ArrayList<StateNodes> node) {
		...
		for (int i = 0; i < connections.size(); i++) {
			...
			for (int t = 0; t < node.get(locate).stateConnections.size(); t++) {
				String temp = node.get(locate).stateConnections.get(t);
	...
	public void sort_Connections(ArrayList<StateNodes> node) {
		...
		for (int j = 0; j < node.size(); j++) {
			Collections.sort(node.get(j).stateConnections);
		}
	...
	public void Matching_Connections_For_All(ArrayList<StateNodes> nodes) {
		for (int i = 0; i < nodes.size(); i++) {
			for (int k = 0; k < nodes.get(i).stateConnections.size(); k++) {
				
				if (k != 0) {
					
					String last_State = nodes.get(i).stateConnections.get(k - 1);
					last_State = last_State.substring(0, 1);
					
					
					String current_State = nodes.get(i).stateConnections.get(k);
					current_State = current_State.substring(0, 1);
					
					if (current_State.equals(last_State)) {
						
						current_State = nodes.get(i).stateConnections.get(k);
						last_State = nodes.get(i).stateConnections.get(k - 1);
						current_State = current_State.substring(1);
						if (!(last_State.substring(1).equals(current_State))) {
							
							last_State = last_State + "," + current_State;
							
							nodes.get(i).stateConnections.add(k - 1, last_State);
							nodes.get(i).stateConnections.remove(k);
							nodes.get(i).stateConnections.remove(k);
						} else {
	...
	public void NFA_Combine_States(int h) {
		if (NFA_State_Nodes.get(h).NFA_States_Holder.isEmpty()) {
			...
			for (int i = 0; i < states.get(location).stateConnections.size(); i++) {
				String temp = states.get(location).stateConnections.get(i);
	...
	public void NFA_New_States() {
		the_Tracker++;
		for (int i = 0; i < NFA_State_Nodes.get(the_Tracker).stateConnections.size(); i++) {
			String connection = NFA_State_Nodes.get(the_Tracker).stateConnections.get(i);
			connection = connection.substring(1);
	...
	public void add_NFA_State(int location) {
		...
		for (int i = 0; i < states.get(location).stateConnections.size(); i++) {
			String element = states.get(location).stateConnections.get(i);
	...
	public void double_Check_For_NFA() {
		for (int i = 0; i < states.size(); i++) {
			for (int k = 0; k < states.get(i).stateConnections.size(); k++) {
				if (k != 0) {
					String last_Element = states.get(i).stateConnections.get(k - 1);
					last_Element = last_Element.substring(0, 1);
					String current_Element = states.get(i).stateConnections.get(k);
